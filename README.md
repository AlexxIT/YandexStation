...
**Внимание**. В тексте часто используются термины локальная колонка, локальный режим и локальное управление (они несут одинаковый смысл). Внимательно изучите какие колонки его поддерживают. Функции и возможности локального управления сильно превосходят облачное управление. Локальный режим включается автоматически на поддерживаемых колонках, подробнее [тут](#статический-ip-для-локального-режима).
...
| [Яндекс.Станция Лайт](https://yandex.ru/alice/station-lite)                       | Поддерживается       | Поддерживается |
...
| [JBL Link Music](https://ru.jbl.com/LINK+MUSIC+YANDEX+AI-.html)                   | Нет                  | Поддерживается |
| [JBL Link Portable](https://ru.jbl.com/LINK+PORTABLE+YANDEX+AI-.html)             | Нет                  | Поддерживается |
...
**Внимание**. Ниже пример нестандартной карточки медиаплеера. Её нет в Home Assistant по умолчанию. Она легко ставится через [HACS](https://github.com/kalkih/mini-media-player). Если у вас сложности с установкой - просто используйте стандартную карточку. Она тоже работает.
...
Выбрать станцию можно указав `entity_id` или `device` (для обратной совместимости). В качестве `device` может быть название станции или идентификатор. Можно посмотреть в приложении Яндекс или в [веб](https://quasar.yandex.ru/skills/iot) интерфейсе.
...
Для работы функционала должен быть настроен компонент [Yandex Smart Home](https://github.com/dmitry-k/yandex_smart_home) версии 0.1.4 (или новее) для интеграции Home Assistant в платформу умного дома Яндекса.
...
yandex_station:
  ...
  intents:
    ...
    Какая температура в комнате:
      extra_phrases:  # максимум 3
        - Расскажи температуру в комнате
        - В комнате тепло
      say_phrase: Уточняю
    Какая влажность в комнате:
      say_phrase: Сейчас расскажу
    Как там в доме:
