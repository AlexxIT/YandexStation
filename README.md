...
- [Проигрывание локальных MP3](#проигрывание-локальных-mp3)
...
## Проигрывание локальных MP3

**Только для локального режима!**

Аналогично функции [проигрывание медиа по ссылкам](#проигрывание-медиа-по-ссылкам), но ссылка должна быть на mp3-файл. Ссылка обязательно должна заканчиваться на `.mp3`. Допускается начилие в ссылке любых query-параметров `.mp3?authSig=...`.
...
data:
  media_content_id: 00 FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF 13 E8
  media_content_type: draw_animation
target:
  entity_id: media_player.yandex_station  # замените на вашу колонку
```

У колонки есть 32 управляемых светодиода. Для контроля которыми используется 35 чисел в 16-ричном формате. Первое число всегда `00`. Последние два числа это задержка отображения кадра в миллисекундах (`0x13E8` = 1000мс = 1сек). Остальные числа это яркость светодиода (`00` - выключен, `FF` - максимальная яркость). Для анимации просто указывайте кадры один за другим подряд. Пробелы игнорируются.
