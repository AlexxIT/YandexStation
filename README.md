...
yandex_station:
  include:
  - name: Кондиционер  # имя вашего кондиционера
    temp_sensor: sensor.temperature  # имя сенсора HA если необходимо получать текущую температуру с другого сенсора (необязательный параметр)
